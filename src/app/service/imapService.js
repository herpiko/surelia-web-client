...
ImapService.prototype.listBox = function(boxName, canceler) {
  var self = this;
  var promise = self.$q.defer();
  if (self.$rootScope.listBoxCanceler) self.$rootScope.listBoxCanceler.resolve();
  ...
  if (canceler) req.timeout = self.$rootScope.listBoxCanceler.promise;
  self.$http(req)
  .success(function(data, status, headers) {
...
ImapService.prototype.retrieveMessage = function(id, boxName, canceler) {
  var self = this;
  var promise = self.$q.defer();
  if (self.$rootScope.retrieveMessageCanceler) self.$rootScope.retrieveMessageCanceler.resolve();
  ...
  if (canceler) req.timeout = self.$rootScope.retrieveMessageCanceler.promise;
  self.$http(req)
  .success(function(data, status, headers) {
    promise.resolve(data, status); 
  })
