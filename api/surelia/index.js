              ...
              smtp.auth(result.username, password)
                .then(function(){
                  ...
                  newMessage.build(function(err, message){
                    if (err) {
                      return reply({success : false, error : err.message}).code(500);
                    }
                    console.log(message);
                    smtp.send(payload.sender, payload.recipients, message)
                      ...
                      .catch(function(err){
                        reply({success : false, error : err.message}).code(500);
                      })
                  })
                })
                .catch(function(err){
    ...
    if (pool.map[id]) {
      ...
      if (pool.map[id].obj.client.state == "disconnected") {
        ...
        client.connect()
          ...
          .catch(function(err){
            if (err) {
              return reply({success : false, err : err.message}).code(500);
            }
          })
      } else {
...
ImapAPI.prototype.listBox = function(request, reply) {
  var realFunc = function(client, request, reply) {
    ...
    client.listBox(request.query.boxName)
      ...
      .catch(function(err){
        reply({success:false,err : err.message}).code(500);
      })
  }
  checkPool(request, reply, realFunc);
}
