    ...
    client.retrieveMessage(request.query.id, request.query.boxName)
      .then(function(message){
        ...
        var attachments = {
          ...
          timestamp : message.parsed.date
        }

        // First, check if there is existing attachment in db
        attachmentModel().find({username : attachments.username, timestamp : attachments.timestamp}).exec(function(err, result){
          if (err) {
          ...
          if (result && result.length == 0) {
            attachmentModel().create(attachments, function(err, result){
              ...
              message.attachmentId = result._id;
            ...
            message.attachmentId = result[0]._id;
            reply(message);
          }
        })
      })
...
ImapAPI.prototype.getAttachment = function(request, reply) {
  var realFunc = function(client, request, reply) {
    attachmentModel().findOne({_id : request.query.id}).exec(function(err, result){
      if (err) {
        return reply(err).code(500);
      }
